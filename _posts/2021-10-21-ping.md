...
Actually, about 85% of addresses will be in the geolocation IP database, and 10% will return a response.
...
However old data doesn't affect the data much. Using an older database shifted points by 2&deg; on average, well within a graticule's limits.
...
So on average, each block contains 450 IP addresses. Again I will say, IPv4 geolocation data is not accurate.
...
After removing the defaults, this is what the frequency distribution for all 3.5 million unique IPv4 blocks looks like, averaged to 1&deg;&#215;1&deg; graticules:
<figure class="post-figure">
<img class="img-95"
    src="/assets/posts/ping/Frequencies_MaxMind_1deg_no_defaults.png"
	alt="frequency of 60,000 ping locations"
	>
<figcaption></figcaption>
</figure>
With defaults: it would be all blue except for one red square in Kansas.
...
[^graticules]: The circle of latitude decrease in size with increasing latitude, so the graticules get smaller if you keep the angles constant. Have a look at a globe for a better idea.
	Each graticule will have a height of $\frac{\pi R}{N}$ but a varying width of $\frac{2\pi R}{2N}cos(\phi)$ where $\phi$ is the latitude. The area can be found by projecting it onto a cylinder. Hence the area will be $(Rsin(\phi_1) - Rsin(\phi_2))(\frac{\pi R}{N}) = \frac{1}{N}\pi R^2(sin(\phi_1) - sin(\phi_0))$