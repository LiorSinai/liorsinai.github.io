...
These will then be normalsied to values between 0 and 1 using the softmax function.
...
That post was lacking however in that in did not create a generative transformer.
...
You can also use cURL to download it:
...
Optionally, the small amount of non-acsii characters (œ, Æ,æ, …) should be removed. I also removed the "&" symbol and changed the archaic usage of "&c." to "etc.".
...
It is recommend to load/add the following packages for this tutorial:
...
<div class="card">
  <div class="card-body">
    ...
    <ul>
        <li> Sinusodial Position Encodings: The original paper gave equations to calculate a fixed embedding matrix with the fairly generic properties that each column is sufficiently distinct to every other column but columns close to each other share some similarity.
...
<div class="card">
  <div class="card-body">
    ...
    This includes sparse attention layers, factorisation/kernels for linear attention and downsampling.
...
From this we can see that the first matrix product is a weighted [dot product][WikiDotProduct] of every vector to every other vector in the input matrix, resulting in a $n \times n$ matrix:
...
The [Attention][Attention_2017] paper suggested a further enhancement on attention where the input matrix is divided amongst $H$ heads. This results in a $\tfrac{d_\text{emb}}{h} \times n \times H$ array.
...
Later we will be pass the same value `x` for all of them.
...
2. The dimension $d$.
3. The hidden dimension for the feedforward network. The convention is $4d$.
...
This has 42,439 parameters:
{% highlight julia %}
sum(length, Flux.params(model)) # 42,439 parameters
{% endhighlight %}
...
A better approach is a randomly sample based on the probabilities.
That way a token with a high probability is more likely to be chosen, but it is not guaranteed.
This gives us some variation in the results.
...
The model will be trained on segments of the text which match the context length $c$.
For a text of length $L$ there are $L-c+1$ characters we can select to be the first character of the segments, excluding the last $c-1$ characters.
...
Any character at any point in the text except for the end will have a probability of appearing of $p\approx c/L$ (the ends are less likely).
For many steps $s$ this binomial distribution can be approximated with a normal distribution with a mean $sp\approx sc/L$ and standard deviation $\sqrt{sp(1-p)}\approx \sqrt{sc/L}$. For example, for 4.9 million characters, a context length of 64 and 100,000 steps, each character at each point will appear 1.31±1.14 times.
...
Like other types of averages, perplexity does not describe the spread of the distribution well and outliers can have an outsized effect on it.
...
We'll put the first 95% of data in the train split and the remainder in the validation split.
...
This works well enough, but will require many more steps to train. I recommend at least 10 epochs, where one epoch is defined as $0.95L/(cB)$ steps. 
(Based on the logic in [Batch Generation](#batch-generation) this ensures each character at each position in the text appears approximately once per epoch.)
For $L=4.9\times10^6$, $c=64$ and $B=32$, this is 2,300 steps per epoch.

Please see my [training.jl](https://github.com/LiorSinai/TransformersLite-Examples/blob/main/common/training.jl) file for a more involved function which also does the following:
