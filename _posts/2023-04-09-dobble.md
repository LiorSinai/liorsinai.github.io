...
_Update 24 April 2023: Corrected calculation for randomly evenly distributed probabilities. The original assumed P(card in different bag) = P(different bag). These quantities are however only approximately equal._
...
The probability that the card is in a different bag is 

$$
\begin{align}
P(\text{card in different bag}) &= \frac{N-p}{N-1} \\
              &= \frac{(bp)-p}{(bp)-1} \\
              &= \frac{b-1}{b-\frac{1}{p}} \\
...
$$
\begin{align}
P(Y=k | k >0) &= P(\text{card in different bag}) \cap P(X=k) \\
...
$$
\begin{align}
P(Y=0) &= P(\text{card in same bag}) \cup [P(\text{card in different bag}) P(X=0)] \\
