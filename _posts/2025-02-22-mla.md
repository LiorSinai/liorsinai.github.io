...
However, DeepSeek does not provide evidence to the contrary: that the compression results in losses that affect quality.

The compression also results in significant performance boost, which DeepSeek strangely does not mention in their [paper][DeepSeek_V2].
Note that in MLA there are three matrix multiplications to perform comprising of $2(2d_hH + d)d_c nB$ scalar operations 
instead of two matrix multiplications comprising of $4(d_h H)d n B$ operations in MHA.[^matrix_complexity]
With the standard $d = d_h H$ and a compression ratio $r=\tfrac{d_h H}{d_c}$, the ratio of operations with MLA compared to MHA is $\tfrac{3}{2r}$.
Therefore this requires only $r>1.5$ for a performance boost too.
The only performance penalty is the marginal storage of the $n$th $c^{KV}$ vector, which is $d_c \times 1 \times B$.
