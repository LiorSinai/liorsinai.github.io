...
- Theory
    - Multi-Head Latent Attention: reduce memory usage during inference with a computation penalty.
...
The aim of this post is to explore one of these ideas in depth, namely Multi-Head Latent Attention (MLA).
It reduces memory during inference at a computational cost.
...
As we will see, it actually combines 5 ideas:
1. Attention and multi-head attention from [Attention is all you need (2017)][Attention_2017].
2. Low-Rank Adaption matrices (LoRA) from [LoRA: Low-Rank Adaptation of Large Language Models (2021)][LoRA_2021].
3. KV Caching.
4. Weight absorption.
5. Rotary position embeddings (RoPE) from [RoFormer: Enhanced Transformer with Rotary Position Embedding (2021)][RoPE_2021].
...
This was the case for my original [generator blog post][generator] based on Andrej Karpathy's [Zero to Hero course][ZeroToHero].
...
Focusing on one head with dimension $d_h$ for one batch, the input matrix $X$ is split into the first $n-1$ columns and the $n$th column.
...
As we will soon see, this comes at the cost of increasing computation.
...
However, there are now three matrix multiplications to perform comprising of $2(2d_hH + d)d_c nB$ scalar operations instead of one comprising of $2(d_h H)d n B$ operations.[^matrix_complexity]
...
Create a test layer with compression ratio $d_hH/d_c=4$:
{% highlight julia %}
...
Recognising that MLA increases the computation cost, DeepSeek suggested a way to decrease it by absorbing weight matrices into each other.
...
However where $W^{KQ} \in \mathbb{R}^{d_c \times d_c \times H}$, $W_OV \in \mathbb{R}^{d_o \times d_c H}$ is a typical 2D matrix.
...
<h3 id="mla-absorption-code">3.4 Code</h3>
<h4 id="broadcasted-batched-mul">3.4.1 Broadcasted batched multiplication</h4>
...
<h4 id="mla-absoprtion-code-code">3.4.2 Absorption</h4>
...
The first step is create the $W^{KR}$ and $W^{OV}$ matrices.
...
<h3 id="mla-rope">3.5 RoPE</h3>

The last enhancement DeepSeek adds is rotary positional embeddings [RoPE][RoPE_2021].
...
Thank you for following through with this blog post.
...
[^matrix_complexity]: This is the naive algorithm. For sizes $n \times d$ and $d \times m$, there are $d$ multiplications for each of the $nm$ elements and $d-1$ additions (no addition for the first element), so there are $nm(2d-1)$ operations in total.