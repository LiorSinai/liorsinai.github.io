...
- I have not included the outer loop which loops over the test set. It is slightly unusual that the source code does this. 
...
Creating a 120,000&times;7,600 distance matrix for this dataset took 3 hours 13 minutes on a 2.30 GHz processor with 16 threads with 16GB of RAM. The matrix was stored in an Arrow file with a size of 6.8GB. 
...
- What is the influence of different tie break strategies?
...
The curves are fitted based on the following equation ([source][Rosenfeld2019]):[^linear_regression]

$$
\begin{align}
    \epsilon &= e^c n^b = a n^{-\alpha} \\
    \implies \log(\epsilon) &= b \log(n) + c
\end{align}
$$

where $\epsilon$ is the error, $1-\epsilon$ is the accuracy and $n$ is the training/reference data size.
...
- Transformer details:
  - Text prepocessing: lowercase, remove unicode characters and punctuation.
  - Vocabulary size: 9,969. All 2 letter words with a document frequency greater than 29. 
  - Embedding dimension: 32.
  - Encoder stacks: 1.
  - Training epochs: 10.
- TF-IDF + LR details:
  - Text prepocessing: lowercase, remove unicode characters and punctuation.
  - Vocabulary size: 18,525. All 2 letter words with a document frequency greater than 10.
  - Training epochs: 10.
...
A simple ranking of weights shows the most important words per class.
- Transformers are more complex but some effort has been to understand their many weights.
- With this GZip + kNN technique the GZip part is the main black box. There is no obvious way I know of to understand why it compresses some samples more without diving into its complexities.
...
This was an interesting technique but it failed to live up to the hype on social media.
This compressor classification may be based on a simple concept but in reality it is clunky to implement.
Implementation is frustratingly slow and unnecessary when older techniques can be done 100 times quicker with greater accuracy.
The paper set high expectations too but the utilisation and defense of a questionable accuracy method seriously detracts from it.

That said, I hope this article does not come across as too negative.
The method is novel and interesting and it does work.
It could lead to other interesting research.
We'll have to see.
...
[^slight_randomness]: There is a small amount of randomness remaining from the sorting.
    ...
    Shared scores can happen if the following criteria are met:

    - The test sample $x$ is long so that $C(x)>C(y_j)$ for many reference samples.
    - The difference $C(xy)-C(y_j)$ is the same for a few reference samples.

    This will result in the same NCD score across multiple $y_j$ samples.
...
[^linear_regression]: Fitting a linear regression curve is very easy and I worry not well known enough. The function is:
    ...
    ```
    where the `\` operator will make use of Moore–Penrose inverse $A^+=(X^T X)^{-1}X^T$. This is the optimal solution to minimise the mean squared error.
    For the logistic regression curve, pass in `(log.(x), log.(y))` in instead.
